...
class Settings(BaseSettings):
    ...
    app_name: str = Field(default="Test Code MCP", alias="CODETEST_MCP_APP_NAME")
    host: str = Field(default="0.0.0.0", alias="CODETEST_MCP_HOST")
    port: int = Field(default=8100, alias="CODETEST_MCP_PORT")
    #: MCP 전송 방식. http (Agent 가 원격 호출) 또는 stdio (자식 프로세스로 기동)
    transport: str = Field(default="http", alias="CODETEST_MCP_TRANSPORT")
